...
func GetUsers(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Query("SELECT id," +
		"judul_buku, jumlah, nama_peminjam, alamat_peminjam, noHp_peminjam, tanggal_pinjam, tanggal_pengembalian, lama_pinjam FROM peminjamanBuku_naufal")
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
	for result.Next() {
		var user User
		err := result.Scan(&user.ID, &user.JudulBuku,
...
func CreateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	stmt, err := db.Prepare("INSERT INTO peminjamanBuku_naufal(judul_buku, jumlah, nama_peminjam, alamat_peminjam, noHp_peminjam, tanggal_pinjam, tanggal_pengembalian, lama_pinjam) VALUES(?,?,?,?,?,?,?,?)")
	if err != nil {
		panic(err.Error())
	}
...
func GetUser(w http.ResponseWriter, r *http.Request) {
	...
	params := mux.Vars(r)
	result, err := db.Query("SELECT id,"+
		"judul_buku, jumlah, nama_peminjam, alamat_peminjam, noHp_peminjam, tanggal_pinjam, tanggal_pengembalian, lama_pinjam FROM peminjamanBuku_naufal WHERE id = ?", params["id"])
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func UpdateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := db.Prepare("UPDATE peminjamanBuku_naufal SET judul_buku=?, jumlah=?, nama_peminjam=?, alamat_peminjam=?, noHp_peminjam=?, tanggal_pinjam=?, tanggal_pengembalian=?, lama_pinjam=? WHERE id = ?")
...
func DeleteUser(w http.ResponseWriter, r *http.Request) {
	...
	stmt, err := db.Prepare("DELETE FROM peminjamanBuku_naufal WHERE id = ?")
	if err != nil {
		panic(err.Error())
	}
...
func InitDB() {
	db, err = sql.Open("mysql",
		"root:@tcp(127.0.0.1:3306)/db_2200763_naufalalamsyahramanda_uas_pilkomb")
	if err != nil {
		panic(err.Error())
	}
}
